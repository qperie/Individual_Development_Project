...
public class TirGauche : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        this.touchZone = new Rect(0, Screen.height / 2, Screen.width / 2, Screen.height);
    }
    ...
    void Update()
    {
        if (Input.touchCount > 0)
        {
            ...
            if (touchZone.Contains(touchPosition) || Input.GetKeyDown(KeyCode.LeftArrow))
            {
                if (this.monJoueur.munitions > 0)
                {
                    Debug.Log("Tire avec l'arme GAUCHE");
